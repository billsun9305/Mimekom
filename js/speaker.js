...
function init_show(speaker_show_num){
	...
	speaker_list.some(function(speaker_info) {
		...
		newDiv.find('.text-content').html(speaker_info);
		newDiv.find('.company_text').html(speaker_company);
	    newDiv.find('.speaker_bg').css("background-image", "url(img/speakers/$SPEAKER_NAME.png)".replace("$SPEAKER_NAME",speaker_info.replace(/ /g, '%20'))); 
